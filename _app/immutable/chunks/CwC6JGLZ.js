...
const x = ((_a = globalThis.__sveltekit_r1jjhu) == null ? void 0 : _a.base) ?? "/cassida", Lt = ((_b = globalThis.__sveltekit_r1jjhu) == null ? void 0 : _b.assets) ?? x ?? "", At = "1775396665192", Ge = "sveltekit:snapshot", We = "sveltekit:scroll", Ye = "sveltekit:states", Ut = "sveltekit:pageurl", K = "sveltekit:history", W = "sveltekit:navigation", C = { tap: 1, hover: 2, viewport: 3, eager: 4, off: -1, false: -1 }, xe = location.origin;
function ze(e) {
...
const Ct = /* @__PURE__ */ new Set(["icon", "shortcut icon", "apple-touch-icon"]), $ = Fe(We) ?? {}, z = Fe(Ge) ?? {}, P = { url: qe({}), page: qe({}), navigating: be(null), updated: Tt() };
function Ae(e) {
  $[e] = D();
}
function Nt(e, t) {
...
function ie({ url: e, params: t, branch: n, status: a, error: r, route: i, form: o }) {
  ...
  const l = { type: "loaded", state: { url: e, params: t, branch: n, error: r, route: i }, props: { constructors: jt(n).map((u) => u.node.component), page: $e(k) } };
...
async function it({ id: e, invalidating: t, url: n, params: a, route: r, preload: i }) {
  ...
  for (let g = 0; g < c.length; g += 1) if (c[g]) try {
    ...
    const j = await Wt(g, m, o);
    return j ? ie({ url: n, params: a, branch: m.slice(0, j.idx).concat(j.node), status: S, error: E, route: r }) : await ct(n, { id: r.id }, E, S);
  }
  else m.push(void 0);
...
async function B({ type: e, url: t, popped: n, keepfocus: a, noscroll: r, replace_state: i, state: o = {}, redirect_count: s = 0, nav_token: l = {}, accept: c = Ve, block: f = Ve, event: h }) {
  ...
  const w = N;
  N = l;
  const u = await ue(t, false), p = e === "enter" ? Pe(_, u, t, e) : lt({ url: t, type: e, delta: n == null ? void 0 : n.delta, intent: u, scroll: n == null ? void 0 : n.scroll, event: h });
  if (!p) {
    f(), N === l && (N = w);
    return;
  }
  const m = v, g = R;
  c(), J = true, oe && p.navigation.type !== "enter" && P.navigating.set(Y.current = p.navigation);
  let d = u && await it(u);
  if (!d) {
  ...
  !a && !ft && en(t, !O), Ke = true, d.props.page && Object.assign(k, d.props.page), J = false, e === "popstate" && ot(R), p.fulfil(void 0), p.navigation.to && (p.navigation.to.scroll = D()), G.forEach((b) => b(p.navigation)), P.navigating.set(Y.current = null);
}
  ...
  history.scrollRestoration = "manual", addEventListener("beforeunload", (t) => {
    ...
    if (!ye) {
      if ((_a4 = t.state) == null ? void 0 : _a4[K]) {
        const n = t.state[K];
        if (N = {}, n === v) return;
        const a = $[n], r = t.state[Ye] ?? {}, i = new URL(t.state[Ut] ?? location.href), o = t.state[W], s = _.url ? de(location) === de(_.url) : false;
        if (o === R && (nt || s)) {
...
async function Qt(e, { status: t = 200, error: n, node_ids: a, params: r, route: i, server_route: o, data: s, form: l }) {
  ...
  try {
    ...
    h = await Oe({ status: Le(u), error: await X(u, { url: c, params: r, route: i }), url: c, route: i }), e.textContent = "", w = false;
  }
